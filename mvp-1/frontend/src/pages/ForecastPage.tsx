...
export function ForecastPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['forecast', year, taxForm, zusStage],
    queryFn: () => getFinancialForecast({ year, taxForm, zusStage }),
  })
  ...
  return (
    <div className="p-6">
      ...
      {isLoading && <div className="text-gray-400 text-sm py-12 text-center">Ładowanie prognozy…</div>}
      {isError && <div className="text-red-500 text-sm py-12 text-center">Nie udało się pobrać prognozy.</div>}

      {data && (
        <>
          {/* Karty KPI — prognoza całoroczna */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <Kpi
              label="Prognozowany przychód (rok)"
              value={fmt(data.fullYear.revenue)}
              color="text-blue-600"
              sub={`run-rate ${fmt(data.avgMonthlyRevenue)}/mies.`}
            />
            <Kpi
              label="Prognozowane koszty (rok)"
              value={fmt(data.fullYear.costs)}
              color="text-red-500"
              sub={`run-rate ${fmt(data.avgMonthlyCosts)}/mies.`}
            />
            <Kpi
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-6">
            <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
              <span className="text-sm font-semibold text-gray-700">Prognoza miesięczna {data.year}</span>
              ...
              <table className="w-full text-sm">
                ...
                <tfoot>
                  <tr className="border-t-2 border-gray-200 font-semibold text-gray-800 bg-gray-50">
                    <td className="px-3 py-2">Razem (rok)</td>
