...
export function ExpenseMonthDetailModal({ monthKey, expenses, onClose }: Props) {
  const titleId = useId()
  ...
  const totalPLN = monthExpenses.reduce((acc, e) => acc + e.amountPLN, 0)

  const byCategory = monthExpenses.reduce<Record<string, Expense[]>>((acc, e) => {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            <h2 id={titleId} className="text-base font-semibold capitalize">{monthLabel}</h2>
            <p className="text-xs text-gray-500 mt-0.5">
              {monthExpenses.length} {monthExpenses.length === 1 ? 'wydatek' : 'wydatków'} · łącznie{' '}
        ...
        <div className="overflow-y-auto flex-1 px-5 py-3">
          {monthExpenses.length === 0 ? (
            <div className="text-gray-500 text-sm text-center py-10">Brak wydatków w tym miesiącu.</div>
          ) : (
            Object.entries(byCategory).map(([categoryLabel, catExpenses]) => {
              const catTotal = catExpenses.reduce((acc, e) => acc + e.amountPLN, 0)
              return (
                <div key={categoryLabel} className="mb-5">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-xs font-medium text-gray-500">{fmt(catTotal)} PLN</span>
                  </div>
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 bg-white hover:bg-gray-50">
      <span className="text-xs text-gray-500 w-14 shrink-0">
        {format(new Date(expense.date), 'd MMM', { locale: pl })}
      </span>
