  ...
  if (chartData.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
        <div className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-4">Przychód wg klientów</div>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
      <div className="flex items-center justify-between mb-4">
        <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Przychód wg klientów</span>
          ...
          >
            {chartData.map((entry, i) => (
              <Cell
                ...
                stroke={selectedClient === entry.name ? (dark ? '#e2e8f0' : '#1e293b') : 'none'}
                strokeWidth={selectedClient === entry.name ? 2 : 0}
              />
            ))}
          </Pie>
          <Tooltip formatter={(value) => [fmt(Number(value ?? 0)), 'Przychód']} contentStyle={tooltipStyle} />
          <Legend
            formatter={(value) => (
              <span
                className="text-xs cursor-pointer"
                style={{ color: selectedClient && selectedClient !== value ? legendInactiveColor : legendActiveColor }}
